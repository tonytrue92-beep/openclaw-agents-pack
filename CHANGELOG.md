...
## 2026-06-18 — Ретраи загрузок + честные ошибки + гард индексации

- **`ip_dl` теперь делает 3 попытки** (`--connect-timeout 10 --max-time 25`,
  пауза 2с). РФ-origin без CDN иногда моргает (curl 28); БЕЗ ретраев один блип
  терял файл — клиент на Win10 получил 4 из 5 агентов **без IDENTITY.md** и базу
  знаний 2/11. Одно место чинит ВСЕ загрузки: шаблоны, KB, lib-bootstrap.
- **Честные сообщения об ошибке:** убрано «raw.githubusercontent.com временно
  недоступен» (врало — качали с gateway) → «сервер доставки в РФ, выключи VPN,
  дозалить через --refresh-templates (MEMORY не теряется)».
- **Гард индексации:** `openclaw memory index --force` запускается ТОЛЬКО если
  embedding реально подключён. Без него (клиент отказался от умной памяти) он
  лез на api.openai.com и таймаутил 10с×retry. Теперь — пропуск с пояснением.
- smoke: проверка ретраев в обоих ip_dl + ассерты переписаны на busybox-safe
  `grep -F` (без `grep -A`).

## 2026-06-16 — git не спрашивает GitHub-логин (standalone-guard)

- **`export GIT_TERMINAL_PROMPT=0`** в install-agents.sh. Симптом «клиента просят
  скинуть GitHub-логин» лечится в factory (убран git clone приватного репо), но
  при STANDALONE-запуске установщика агентов любой git к приватному github
  (npm-from-git, установка плагина) тоже мог бы вывести интерактивный
  «Username for 'https://github.com':». =0 → git сразу падает вместо зависания.
  (В чейне из factory уже унаследовано.) smoke-guard на месте.

## 2026-06-16 — Видимый ввод всех токенов/ключей

- **Все токены и ключи в установщике агентов вводятся ВИДИМО** (символы на
  экране), как уже сделано в factory. Решение Антона: скрытый ввод путал
  не-технических клиентов (вставка молча не попадала → ложное «токен пустой»).
  Затронуто: Telegram bot token (был masked `read -r -s` + visible-fallback →
  теперь сразу видимый), OpenAI/embedding API-ключ (`read -rs` → `read -r`).
  Чтение bot token по-прежнему из `/dev/tty` (устойчивость к factory→agents
  pipe) + retry при пустом. Подписи «символы не отображаются» → «символы видны».
  smoke-guard: нет ни одного `read -s` (любой токен/ключ виден).
...
- Resume: «следы прошлой установки ≠ принятый токен; безопасно — доустановить (Enter)».
- R1.5: явно «это НЕ подписка ChatGPT и не OAuth — отдельный API-ключ platform.openai.com».
- Финал-оффер: совет «ChatGPT Pro (Plus упирается в лимиты GPT-5.5)».
- Windows/Git Bash: жёсткий хинт «основной путь — WSL; TG ID/code 5/format token = среда, не токен».

## 2026-06-11.3 — Ноль упоминаний моделей в установке (решение Антона)

- R1 полностью тихий: ни заголовка, ни строк про модель (наследование/техдефолт
  пишутся в конфиг невидимо).
- Отсутствие auth больше НЕ стопает установку: «модель выберешь после
  установки», пустой auth-файл уходит в бэкап. Боты подключаются к Telegram,
  финал объясняет: «🧠 Остался один шаг — выбрать модель» + оффер ChatGPT
  (Enter=да) + команды для любого другого провайдера.

## 2026-06-11.2 — Auth-bridge + новая политика моделей (решения Антона)

### Fixed (P0 — «замкнутый круг auth»)
- OpenClaw 2026.6.x хранит auth централизованно в `openclaw.json` — preflight
  больше не требует per-agent `auth-profiles.json`, если центральные профили
  есть («✓ Auth централизованный»); копирование в новых агентов пропускается.
  Разорвана петля «factory: всё ок → agents-pack: иди в factory».
- `agents delete --yes` получил fallback для старых CLI без флага.

### Changed (политика моделей)
- R1 «Выбор модели» убран: агенты наследуют модель системы, без рекомендаций.
- Финал: оффер «Перевести всех агентов на ChatGPT (GPT-5.5)? [Y/n, Enter=да]»
  → запускает openclaw-add-codex (интерактив, не VPS).

## 2026-06-11 — Hermes включён в Pro (VIP)

- Пункт «4) Hermes»: валидный VIP-токен из кэша зачитывается автоматически
  («✓ У тебя Pro — Hermes включён в твой тариф»), при ручном вводе принимаются
  VIP- и HRM-токены. Base/SUB по-прежнему не проходят. HRM-SKU остаётся для
  продажи Hermes отдельно от Pro.
...
### Changed
- Решение Антона: **Cloudflare не используем.** Эндпоинты аналитики
  (`/issue`/`/activation`/`/stats`) хостит технарь на своём сервере (рядом с
  Prodamus-webhook). Референс-реализация и схема: `handoff/analytics-endpoint-reference/`
  (бывший Worker-код из factory). Брифы/STATUS переписаны.
- В установщике поведение не менялось: пинг по-прежнему выключен
  (`VIP_ACTIVATION_ENDPOINT` пуст), включится URL-ом технаря. Текст
  телеметрии уточнён («пишется только локально»).

`INSTALLER_VERSION 2026.06.10.1 → 2026.06.10.2`

---

## 2026-06-10.1 — Аудит R3: live-фиксы (по реальному поведению openclaw)

### Fixed
- **`agent_exists`** — переписан по live-тесту на macOS/BSD grep: `tech`
  больше не совпадает с `tech-2` (раньше — false-positive: «дополнить»
  пропускал установку), убраны нерабочий в BSD `\s` и опасный
  case-insensitive fallback. Паттерн заякорен на реальный формат
  `agents list` («- <id> (Имя)»).
- **Дубль-бот против УЖЕ установленных агентов:** перед R2 собираются
  юзернеймы ботов существующих агентов (live: `channels status --probe`
  отдаёт `bot:@username`) — ввод бота, занятого другим агентом, ловится
  с понятным сообщением (раньше проверялись только дубли текущей сессии).
- **`--refresh-templates` с суффиксованными id** (tech-2): шаблоны качаются
  по роли (tech), workspace остаётся суффиксованным — раньше суффикс уходил
  в URL шаблонов и обновление падало.
- **R0-сценарии честнее:** непересекающийся с установленными выбор больше
  не маскируется под «свежую установку» («это доустановка; в системе уже
  есть: …»); в UPGRADE-списке подсказка про `--refresh-templates`.

`INSTALLER_VERSION 2026.06.10 → 2026.06.10.1`

---

## 2026-06-10 — Аудит R2: фикс чейна для Base + гейты токенов + CI

### Fixed (критичное)
- **Чейн factory→agents-pack для Base-клиентов.** Дочерний установщик снова
  показывал меню тарифа с дефолтом Enter=Pro → STD-клиент по Enter получал
  «несоответствие тарифа» и установка падала. Теперь тариф читается из
  **кэша токена** (которым factory уже подтвердил оплату): меню показывает
  подсказку «Твой тариф: … — просто нажми Enter», дефолт = твой тариф
  (STD→Base, VIP→Pro, SUB→1, HRM→4/Hermes).
- **Опечатка в меню V_MAIN** больше не завершает установку «успехом»
  (exit 0 → чейн считал, что агенты поставлены): теперь до 3 перепросов,
  затем exit 1.
- **HRM-токен** (Hermes SKU) больше не открывает Base-установку — graceful
  exit с подсказкой «Hermes — пункт 4 меню».
- **`--config` с STD-токеном** больше не получает ложный отказ по тарифу:
  VIP_MODE из config теперь по префиксу токена, а не по факту наличия VIP_TOKEN=.
- Совет в отказе Base+SUB: «опция 3» → «опция 1» (3 — это Pro).
- stale-env unset покрывает BOT_TOKEN_LEADCLOSER/CONTENT; tty-guard не
  умирает молча под nohup; кэш токена с недоступным stat — fail-closed (chmod 600).

### Fixed (CI/релиз)
- SHA256SUMS и CI-проверка покрывают **все** шаблоны (find вместо глоба —
  раньше 21 файл skills/knowledge был вне контроля целостности).
- release.yml: sed подстановки INSTALLER_COMMIT заякорен (не задевает guard) + проверка.
- ci.yml: paths включают docs/dmg-template/handoff; shellcheck — глоб всех скриптов.
- build-bundle: обратная проверка LIB_ORDER (новый lib-модуль не потеряется).
- docker-чек шаблонов: ровно одно ожидаемое значение (85).

### Polish/docs
- Pro-меню без дубль-строки агентов; --help/--only знают leadcloser/content;
  vip-install-guide переписан под 8 агентов и пункт «3) Pro» (раньше учил
  выбирать «2», что = Base!); windows-guide — актуальное меню + токен;
  README — токен-шаг + актуальная дефолт-модель; walkthrough помечен
  устаревшим; curator-guide — версии/тег.

`INSTALLER_VERSION 2026.06.09.1 → 2026.06.10`

---

## 2026-06-09 — Чистка: удалён осиротевший install-trial.sh (H6)

### Removed
- `scripts/install-trial.sh` (+ его smoke-блок wave 30–44). Это был **устаревший
  дубль** тест-драйва (pre-TRY, «бесплатно без токена»), противоречащий канону.
  Единственный тест-драйв-установщик — репо **openclaw-test-drive** (по TRY-токену).
  install-agents.sh не затронут (версия без изменений).

---

## 2026-06-09.1 — Аудит-фиксы (H1/H2/H4/M1 + полировка)

### Fixed
- **H1:** `--config` с `VIP_TOKEN=` теперь доходит до валидатора (мост
  `VIP_TOKEN→COURSE_TOKEN`) — неинтерактивная VIP-установка больше не падает.
- **H2 (security):** `--only leadcloser` / `--only content` (Pro-агенты) теперь
  под тариф-гейтом — нельзя поставить по STD-токену.
- **H4:** активационный пинг шлёт `sha256(полного токена)` (канон, совпадает с
  ботом `/issue`), а не embedded hash16 — склейка в D1 сойдётся.
- **H7:** убран мёртвый endpoint `aiteam-vip.openclaw.ai`; по умолчанию пинг
  **выключен** (no-op), активируется заданием `VIP_ACTIVATION_ENDPOINT` после
  деплоя Worker; в payload добавлены `tier`/`track`.
- **M1:** дефолт-модель агентов — `opencode/minimax-m2.5-free` (рабочая,
  бесплатная) вместо legacy `openai-codex/gpt-5.4`; ChatGPT — opt-in `openclaw-add-codex`.

### Polish
- Меню Pro показывает все 8 ролей (были видны 3). Текст «ТРИ РАЗНЫХ бота» → по
  числу агентов. KB `extraPaths` подключается идемпотентно (не затирает чужие пути).

`INSTALLER_VERSION 2026.06.09 → 2026.06.09.1`
...
- На Pro-установке клиент выбирает агентов из 8 нумерованным списком
  ...
  меньше бот-токенов (ниже флуд-риск Telegram). Уже стоящие агенты помечены `✓`.
- Гейты: меню только в интерактиве и только для Pro; при `--only` / `--install`
  / `--vps` / non-TTY ставятся все 8 (обратная совместимость 100%).

`INSTALLER_VERSION 2026.06.06 → 2026.06.09`
...
- Платный поток теперь запускается **одной командой** из factory (STD/VIP
  дотягивают этот установщик автоматически с тем же `--course-token`).
  Отдельный запуск agents-pack по-прежнему работает (перезапуск / `--refresh-templates`).

`INSTALLER_VERSION 2026.06.04.3 → 2026.06.06`

---

## 2026-06-04 — Второй установщик всегда тянет последнюю версию OpenClaw

### Триггер

Антон: установщик должен ставить **абсолютно свежий** движок. Если есть
новая версия OpenClaw — сразу её ставить/обновлять.

### Added

- Перед установкой агентов (основной install-путь, после preflight)
  установщик **всегда** прогоняет `npm install -g openclaw@latest` —
  ставит или **обновляет** движок до последней версии, даже если OpenClaw
  уже стоял (например, поставлен factory давно). Идемпотентно; без npm/сети
  не падает (продолжает с текущей версией). В `--refresh-templates` и
  `--diagnose-only` НЕ запускается (они «ничего не меняют»).
- Закрывает пробел: раньше второй установщик движок не обновлял, и новые
  фичи (база знаний/extraPaths, вход Codex, фикс memorySearch) могли не
  работать на старом движке.

Контекст: factory (1-я ступень) уже обновлял движок до latest; тест-драйв
тоже переведён на «всегда обновлять» (репо openclaw-test-drive).

`INSTALLER_VERSION 2026.06.04.2 → 2026.06.04.3`

---

## 2026-06-04 — Предупреждение про flood-блок @BotFather (создание ботов)

### Триггер

Антон: на Pro нужно много ботов (до 8). Если создавать токены через
`/newbot` подряд за минуту, Telegram может временно (≈сутки) заблокировать
создание новых ботов.

### Added

- В шаге **R2 (TELEGRAM BOT TOKENS)** перед сбором токенов — заметное
  предупреждение: сколько ботов нужно (динамически = число агентов),
  «не создавай всех подряд», совет создавать **партиями по 2-3 с паузой
  10-15 мин». Установщик запрашивает токены по очереди (`read` блокирует),
  так что клиент может делать паузы сам.
- smoke-assert на наличие предупреждения.

`INSTALLER_VERSION 2026.06.04.1 → 2026.06.04.2`

---

## 2026-06-04 — База знаний (мини-вики) для агентов

### Added
- **`templates/knowledge/` — 11 заметок** (сокращённые выжимки из курсовой
  базы, обезличенно): скрипт созвона, матрица возражений, дожим/follow-up,
  продающие смыслы, формула оффера, прогрев, структура вебинара, воронки,
  трафик, кастдев, продуктовая линейка.
- **Подключение к агентам:** `setup_knowledge_base()` (в `lib/agents.sh`)
  качает заметки в `~/.openclaw/knowledge/`, прописывает
  `agents.defaults.memorySearch.extraPaths` (общая база для ВСЕХ агентов —
  проверено dry-run: схема принимает extraPaths) и запускает
  `openclaw memory index --force`. Агенты находят знания через `memory_search`.
- Разворачивается в **Pro** (вместе с полной командой) — `install-agents.sh`
  шаг R4.5, гейт по `VIP_MODE`.
- 5 агентов (leadcloser, content, marketer, producer, copywriter) в AGENTS.md
  знают про базу и ищут в ней перед стратегическим ответом.

### Принцип
База **сокращённая** (выжимки, не полные скиллы) и **обезличенная** (без
имён авторов-методологов и личных маркеров — проходит security-audit).
Полные версии остаются у владельца.

### Изменения
- `tests/docker/run-checks.sh`: template_count `85`. Новый smoke-блок (наличие
  11 заметок + механизм extraPaths + вызов в установщике).

`INSTALLER_VERSION 2026.06.04 → 2026.06.04.1`

---

## 2026-06-04 — Два новых Pro-агента: Лидоруб (продажи) + Контент (видео)

### Added
- **💰 Лидоруб** (`leadcloser`) — менеджер по продажам: закрытие заявок,
  ответы клиентам, ведение переписки. Скиллы: `deal-closing`,
  `client-replies`, `crm-integrations`. Последний — **карта готовности**
  к интеграциям (GetCourse / amoCRM / Bitrix24 / Salebot / Chatplace):
  что подключается, какой ключ, куда вставить. Живое подключение — позже.
- **🎥 Контент-агент** (`content`) — картинки (как дизайнер) + видео
  (Hyperframes + HuggingFace) + озвучка (ElevenLabs) + сведение в ролик.
  Скиллы: `image-generation`, `color-palette`, `video-hyperframes`,
  `voiceover-elevenlabs`, `video-assembly`. Видео/озвучка — по API-ключу
  (готовность); без ключа агент готовит сценарий и раскадровку.

Оба агента — **только в Pro (VIP)**. Pro: 6 → **8 агентов**. Base (3) не тронут.
Установщик при Pro теперь просит 8 бот-токенов (по одному на агента).

### Принцип «готовность, не обман»
Интеграции (CRM, видео, озвучка) пока НЕ подключаются вживую — это
отдельная будущая волна (особенно OAuth у amoCRM/Bitrix24). Агенты в
персонах/скиллах честно говорят клиенту, что отправка/публикация/рендер
во внешние системы подключается позже по ключу.

### Changed
- `scripts/lib/agents.sh`: `find_installed_agents`, `skills_list`,
  extras-гейтинг — добавлены `leadcloser` и `content`.
- `scripts/install-agents.sh`: `--only` allowlist, VIP `AGENTS_TO_INSTALL`,
  эмодзи/лейблы (R2 + финал), счётчик «6 → 8 агентов».
- Тесты: новый smoke-блок (шаблоны/скиллы/код новых агентов);
  `tests/docker/run-checks.sh` — template_count `74`.
- Доки (README/гайды/cheatsheet): счётчик 6 → 8, перечень дополнен.

`INSTALLER_VERSION 2026.06.02 → 2026.06.04`
...
Антон: «в конце установки нужно, чтобы так же, как открывается сайт
OpenCode, открывался наш сайт-продажник — тот, что мы кидаем ссылкой».

### Что изменилось

В финале установщик теперь **автоматически открывает в браузере**
`${COURSE_URL}` (страница полной версии) — тем же паттерном, что и
opencode.ai на Шаге 2 (`open` на macOS / `xdg-open` на Linux,
в фоне, ошибки подавлены). Клиент сразу видит продающую страницу,
а не просто ссылку текстом в терминале.

### Версия

`TRIAL_VERSION 2026.05.28.12 → 2026.05.28.13`

---

## 2026-05-28 — Wave 43 (trial: фиксированная модель DeepSeek, без меню)

### Триггер

Антон: «при выборе модели надо убрать MiniMax, убрать GPT-5 — ничего
этого не нужно, нужно чтобы сразу выбрал `opencode/deepseek-v4-flash-free`».

### Что изменилось

Меню выбора модели из wave 42 убрано. Тест-драйв теперь всегда ставит
бесплатную **DeepSeek Flash Free** (`opencode/deepseek-v4-flash-free`) —
без вопросов клиенту. Меньше шагов, меньше путаницы.

- Шаг 2: вместо меню «1/2/3» — сразу `TRIAL_MODEL=opencode/deepseek-v4-flash-free`.
- Тексты и комментарии T2 обновлены (MiniMax → DeepSeek Flash Free).

Анбординг на русском (wave 42, Шаг 5) и вся страховка auth/gateway
(waves 37/40/41) остаются без изменений.

### Версия

`TRIAL_VERSION 2026.05.28.11 → 2026.05.28.12`

---

## 2026-05-28 — Wave 42 (trial: выбор модели + анбординг на русском)

### Триггер

Антон: «нужно чтобы человек выбирал себе первично модель и проходил
быстрый анбординг… чтобы была команда on board… только перевести его
на русский, чтобы человек, который нихуя не понимает в техничке, просто
прошёл для себя быстрый анборд». В его логе ручной `openclaw onboard`
дал «Gateway: reachable» — то есть onboard надёжно поднимает систему.

### Что добавилось

**1. Меню выбора модели (Шаг 2).** Перед вводом ключа клиент выбирает
мозги ассистента:

```
1) MiniMax Free   (бесплатно — для теста)  ← рекомендуется
2) Claude Sonnet  (платно — умнее)
3) GPT-5          (платно)
```

Enter = MiniMax Free. Выбор пишется в `agents.defaults.model.primary`.

**2. Прогон штатного `openclaw onboard --non-interactive` (Шаг 4).**
Запускаем мастер в авто-режиме (`--auth-choice opencode-zen
--opencode-zen-api-key … --skip-channels/search/hooks/skills/ui`),
с `</dev/null` чтобы не повис на промпте. Это то, что в логе Антона
дало «Gateway: reachable».

**3. Видимый русский онбординг-чеклист (Шаг 5).** Вместо английского
TUI — простой ✓/✗ по каждому узлу: движок, модель, Telegram, агент,
gateway. Человек сам видит «Онбординг пройден: 5/5 — всё работает!».

### Надёжность (страховка НЕ потеряна)

`onboard --non-interactive` не проверен на чистой машине, поэтому он
НЕ единственная гарантия. Проверенные пути waves 37/40/41 остаются:
auth-profile пишется напрямую в agent dir; gateway поднимается
`config set gateway.mode local` → `gateway install` → `launchctl
bootstrap` → `start`. Команды идемпотентны — если onboard уже всё
сделал, это no-op.

### Версия

`TRIAL_VERSION 2026.05.28.10 → 2026.05.28.11`
