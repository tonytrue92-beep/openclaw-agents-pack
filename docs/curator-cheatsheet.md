...
> 📍 **Актуальный единый контекст — в [`docs/curator-guide.md`](./curator-guide.md)**
> (все 3 трека, текущие версии и свежие ошибки на 2026-06-06). Этот файл —
> исторический deep-dive по сценариям; версии/состав агентов ниже могли
> устареть, сверяйся с мастер-инструкцией.
...
| `docs/vip-install-guide.md` | VIP пошагово (8 агентов) | Клиент купил VIP, ставит впервые |
...
### СЦЕНАРИЙ 4а: «Бот молчит на VPS, Gateway циклически рестартится»

**Конкретный симптом:**
- Установка прошла, бот живой в @BotFather
- Gateway вроде бы запускается, но через ~45 секунд падает и перезапускается
- Бот не успевает стабильно подключиться к Telegram → молчит на сообщения
- В логах (`openclaw logs --tail 50 --follow`) есть строки `CIAO PROBING CANCELLED`
- Машина — VPS (Ubuntu / Debian / cloud-сервер)

**Причина:** плагин `bonjour` (mDNS) пытается анонсировать Gateway в
локальной сети для авто-обнаружения iOS/Mac-приложением OpenClaw.
На VPS «локальной сети» в этом понимании нет → плагин падает →
gateway рестартится по кругу.
...
После — бот заработает в течение минуты.

**Для wave 10.1+ установщиков:** этот фикс делается **автоматически**
если запустить установщик с флагом `--vps`. Если клиент уже стоит
без `--vps` и наткнулся на проблему — дай команду выше.

**`--diagnose-only`** в wave 10.1+ предупредит если bonjour enabled
на Linux/WSL — куратору проще локализовать.
...
### СЦЕНАРИЙ 7: «Хочу чтобы агенты общались между собой»

**Два механизма** (можно использовать оба или один):

#### A) Group-mode (wave 8) — общая TG-группа

Все боты в одной группе с пользователем — все сообщения видны.
Хорошо для брейнштормов и «совещаний».
...
Полный гайд: `docs/group-mode.md`.

#### B) Bot-to-Bot Communication (Telegram, май 2026) — прямая связь

Боты пишут друг другу напрямую без общей группы. Хорошо для
делегации цепочек («сделай X → передай Y → результат сюда»).

1. У `@BotFather` для **каждого** бота: Bot Settings → включить
   **Bot-to-Bot Communication Mode**
2. Никаких команд установщика — это настройка на стороне Telegram

Полный гайд: `docs/bot-to-bot-setup.md`.

#### Какой когда давать клиенту

- Хочет видеть весь процесс / брейншторм → Group-mode (A)
- Хочет только финальный результат / быстрая делегация → Bot-to-Bot (B)
- Не уверен / спрашивает «что лучше» → начни с Group-mode (привычнее),
  Bot-to-Bot предложи если будут жалобы «слишком много шума в группе»

### СЦЕНАРИЙ 7а: «Боты зациклились — отвечают друг другу бесконечно»

**Симптом:** в чате 5+ сообщений за минуту от ботов без участия
пользователя. Боты отвечают друг другу автоматически.

**Что делать клиенту (срочно):**

1. Открой `@BotFather → /mybots → выбери одного из «зациклившихся»`
2. Bot Settings → выключи **Bot-to-Bot Mode** (Disable)
3. В чате с этим ботом — **«Block Bot»** временно
4. Через минуту — Unblock

**После остановки loop'а — найти причину:**
- Если в `MEMORY.md` или `LEARNING.md` агента есть инструкция вида
  «постоянно проверяй у X не готово ли» — убрать
- Или сделать `--refresh-templates` (вернёт SOUL/LEARNING к
  стабильным версиям, но **не тронет** MEMORY/USER):
  ```bash
  bash <(curl -fsSL .../install-agents.sh) --refresh-templates
  ```

**Защита от повторения:**
- Включить Bot-to-Bot обратно через 5 минут когда понял причину
- Если loop повторяется — отключить совсем и использовать только
  Group-mode (там `requireMention=true` защищает от этого)
...
### СЦЕНАРИЙ 13: «У меня SUB-токен — установщик отказал»

**Не отказал, а graceful-exit.** SUB (subscription) — это базовый
тариф **подписки**, ниже Standard. Включает только:
- OpenClaw движок (через **первый установщик**)
- main-агент (тот же)

**Наш второй установщик (agents-pack)** ставит ДОПОЛНИТЕЛЬНЫХ агентов
(Технарь / Маркетолог / ...). На SUB-тарифе их **нет** — для них
нужен Standard (3 агента) или VIP (8 агентов).

**Что говорить клиенту:**
1. Если он впервые ставит OpenClaw — пусть запустит **первый** установщик
   (`demo-install.sh` openclaw-factory). Там SUB-токен валидируется и
   ставится OpenClaw + main-агент. Этого достаточно для SUB-тарифа.
2. Если main-агент в Telegram уже отвечает — у клиента **всё уже
   установлено** по тарифу. Не нужно ничего больше делать.
3. Если хочет дополнительных агентов — апгрейд на Standard / VIP
   через саппорт-чат курса. После апгрейда новый токен в
   `@AITeamVIPBot`, и agents-pack установщик автоматически распознает.

### СЦЕНАРИЙ 14: «Подписка истекла, бот говорит «expired»»

Бот `@AITeamVIPBot` отказывает в выдаче токена когда `subscription_expires_at < NOW()`.
Клиенту нужно **продлить подписку** через страницу курса. После оплаты
вернуться в `@AITeamVIPBot` → `/start` → получит новый токен.

**Важно:** **уже установленные агенты** (если SUB-клиент успел
поставить main-агент когда подписка была активна) **продолжат
работать локально** — мы не «выключаем» их по истечении. Но клиент
не сможет:
- Переустановить
- Запустить `--refresh-templates`
- Получить новый VIP/Standard-токен пока не продлит
