...
1. Меню — `1) OpenClaw  2) Base (3 агента)  3) Pro (8 агентов)` — выбери
   свой тариф (установщик подскажет его по токену; обычно просто Enter)
2. **Курс-токен** — вставь токен из `@AITeamVIPBot` (если не подхватился сам)
3. **R1** — модель (Enter = бесплатная, карта не нужна)
4. **Выбор агентов** (Pro) — номера через пробел или Enter = все 8
5. **R1.5** — embedding-память (опционально)
6. **R2** — токены ботов (`@BotFather`, по одному на выбранного агента;
   создавай партиями по 2-3 — Telegram блокирует быстрое создание)
7. **R5b** — общая TG-группа (опционально)
