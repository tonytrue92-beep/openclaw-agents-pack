...
Платный клиент запускает **одну команду** с course-token. Factory сам ставит
движок OpenClaw и, если тариф даёт агентов, дотягивает AI-команду в той же
сессии. Отдельную вторую команду клиенту больше не даём.
...
| **Платные агенты** | `openclaw-agents-pack` | Base / Pro — докачиваются автоматически внутри платной команды | 3 агента (Base) или **8 агентов + база знаний** (Pro) |
...
Хочешь мозги поумнее (GPT-5.5) — после установки команда
`openclaw-add-codex`, понадобится обычный аккаунт ChatGPT.
Хелпер ставится установщиком в `~/.openclaw/bin/`:
...
### 5.13e. «Перезапусти команду» — теперь универсальный безопасный совет ⭐ (2026-06-11)
С factory `2026.06.10.2` повторный запуск установочной команды поверх **готовой**
установки ничего не ломает и не переспрашивает: установщик сам видит движок,
подключённый Telegram («✓ Telegram уже подключён: @бот — пропускаю») и созданных
агентов, и сразу едет к доустановке AI-команды по тарифу. Если клиент застрял /
оборвалась сеть / не доехали агенты — первый совет: **запусти ту же команду ещё
раз и жми Enter по шагам**. Токен заново вводить не нужно (кэш). Сменить бота —
только через «Полный сброс» (пункт 4 на шаге настройки).
...
опциональные мозги поумнее: `openclaw-add-codex` (понадобится обычный аккаунт
ChatGPT; это не обязательный шаг установки).
...
3. Для платных проговори одну команду: движок и нужные агенты ставятся в одной сессии по токену.
